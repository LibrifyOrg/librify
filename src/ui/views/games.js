...
import GamePageComponent from "@/ui/components/game/page";
import GameListComponent from "@/ui/components/game/list";
...
export default class GamesView {
	...
	onupdate() {
		if(this.app.ui.game === undefined || this.app.games.has(this.app.ui.game.id)) {
			return;
		} 
		
		this.app.ui.game = Array.from(this.app.games.values()).sort((a, b) => a.data.name.localeCompare(b.data.name))[0];
		m.redraw();
	}
	...
	view() {
		...
		return (
			<div class="games">
				<GameListComponent app={this.app} menuType={this.menuType}></GameListComponent>
				<GamePageComponent app={this.app}></GamePageComponent>
			</div>
		);
	}
}